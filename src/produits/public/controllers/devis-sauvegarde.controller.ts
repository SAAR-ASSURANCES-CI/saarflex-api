...
export class DevisSauvegardeController {
  ...
  @Get('count')
  @ApiOperation({
    summary: 'Compter le nombre de devis sauvegardés de l\'utilisateur',
    description: 'Retourne le nombre total de devis sauvegardés par l\'utilisateur connecté'
  })
  @ApiResponse({
    status: 200,
    description: 'Nombre de devis récupéré avec succès',
    schema: {
      type: 'object',
      properties: {
  ...
  async compterDevis(@Request() req: any): Promise<{ count: number }> {
    return this.devisSauvegardeService.compterDevisUtilisateur(req.user.id);
  }
